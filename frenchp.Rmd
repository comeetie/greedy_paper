...
sol_ms=list()
for (i in 1:nbrun){
  sol_h[[i]]=greed(Xvlegislature$X,K=40)
  sol_h50[[i]]=greed(Xvlegislature$X,K=40, alg=new("hybrid",pop_size=50))
  sol_h100[[i]]=greed(Xvlegislature$X,K=40, alg=new("hybrid",pop_size=100))
  sol_seed[[i]]=greed(Xvlegislature$X,K=40,alg=new("seed"))
  sol_ms[[i]]=greed(Xvlegislature$X,K=40,alg=new("multistarts"))
}
```


```{r frenchp,echo=FALSE, message=FALSE,results="hide",fig.width=5.5,fig.height=4.5}
  library(dplyr)
library(ggplot2)
  ...
  ggplot(gg)+geom_boxplot(aes(x=alg,group=alg,y=icl))+geom_point(aes(x=alg,y=icl))+xlab("")+ylab("ICL")+theme_bw()+ggtitle("French parliament votes")
